...
def _custom_diag_normal_kl(lhs, rhs, name=None):  # pylint: disable=unused-argument
  ...
  """
  with tf.compat.v1.name_scope(name or 'kl_divergence'):
    mean0 = lhs.mean()
    mean1 = rhs.mean()
    logstd0 = tf.math.log(lhs.stddev())
    logstd1 = tf.math.log(rhs.stddev())
...
def feed_forward_gaussian(
  ...
  init_output_weights = tf.compat.v1.keras.initializers.VarianceScaling(
      scale=config.init_output_factor)
  before_softplus_std_initializer = tf.compat.v1.constant_initializer(
  ...
  with tf.compat.v1.variable_scope('value'):
    ...
    for size in config.value_layers:
      x = tf.compat.v1.layers.dense(x, size, tf.nn.relu)
    value = tf.compat.v1.layers.dense(x, 1, None)[..., 0]
...
def recurrent_gaussian(
  ...
  before_softplus_std_initializer = tf.compat.v1.constant_initializer(
      np.log(np.exp(config.init_std) - 1))
  cell = tf.compat.v1.nn.rnn.GRUBlockCell(config.policy_layers[-1])
  flat_observations = tf.reshape(observations, [
      tf.shape(observations)[0], tf.shape(observations)[1],
      functools.reduce(operator.mul, observations.shape.as_list()[2:], 1)])
  with tf.compat.v1.variable_scope('policy'):
    ...
    for size in config.policy_layers[:-1]:
      x = tf.compat.v1.layers.dense(x, size, tf.nn.relu)
    x, state = tf.compat.v1.nn.dynamic_rnn(cell, x, length, state, tf.float32)
    mean = tf.compat.v1.layers.dense(
        x, action_size, tf.tanh,
        weights_initializer=init_output_weights)
    std = tf.nn.softplus(tf.compat.v1.get_variable(
